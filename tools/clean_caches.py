...
def main():
    ...
    targets = ["__pycache__", ".pytest_cache", ".ruff_cache", ".gradle", ".kotlin"]
    ...
    for dirpath, dirnames, filenames in os.walk(root, topdown=False):
        ...
        for f in filenames:
            if f.endswith(".log") or (
                f.endswith(".txt") and not f.startswith("requirements")
            ):
                if (
                    "docs" not in dirpath
                    and ".idea" not in dirpath
                    and ".vscode" not in dirpath
                ):
