...
class ApiService {
    ...
    fun getCachedAuthorProfile(key: String): AuthorResponse? {
        return authorProfileCache[key]
    }

    fun cacheAuthorProfile(key: String, profile: AuthorResponse) {
        authorProfileCache[key] = profile
    }
    ...
    suspend fun searchAuthor(name: String, id: String? = null, forceRefresh: Boolean = false, focus: String? = null): AuthorResponse? {
        ...
        try {
            ...
            val result = decoded.copy(
                field_of_study = sanitizedField,
                expertise = sanitizedExpertise
            )
            if (id != null) cacheAuthorProfile(id, result)
            cacheAuthorProfile(name, result)
            return result
        } catch (e: Exception) {
    ...
    private suspend fun fetchAuthorFromOpenAlex(name: String, id: String? = null, focus: String? = null): AuthorResponse? {
        ...
        return try {
            ...
            cacheAuthorProfile(authorData.id, result)
            cacheAuthorProfile(name, result)
            return result
        } catch (e: Exception) {
    ...
    ): com.company.skolab.model.PaperIntelligence? {
        val base = baseUrl() ?: run {
    ...
    suspend fun getDailyConjecture(authorId: String, name: String): com.company.skolab.model.Conjecture? {
        val base = baseUrl() ?: throw Exception("SkoLab backend services are currently unreachable.")
        try {
            ...
            if (response.status.value == 200) {
                return response.body<com.company.skolab.model.Conjecture>()
            } else {
    ...
    suspend fun chatWithAgent(
        ...
        userMemory: com.company.skolab.data.UserMemoryProfile? = null
    ): String {
                ...
                val memoryMap: Map<String, kotlinx.serialization.json.JsonElement>? = userMemory?.let { mem ->
                    try {
                        ...
                        val jsonObj = json.encodeToJsonElement(
                            com.company.skolab.data.UserMemoryProfile.serializer(), mem
                        )
    ...
    suspend fun syncMemoryEvents(
        userId: String,
        events: List<com.company.skolab.data.ActivityEvent>
    ): Boolean {
        return withContext(Dispatchers.IO) {
    ...
    suspend fun deleteUserAccount(userId: String): Boolean {
        return withContext(Dispatchers.IO) {
            val base = baseUrl() ?: return@withContext false
            try {
                val response = httpClient.delete("$base/api/v1/users/$userId")
                response.status == HttpStatusCode.OK
            } catch (e: Exception) {
                if (e is CancellationException) throw e
                Log.e("ApiService", "deleteUserAccount failed: ${e.message}")
                false
            }
        }
    }
