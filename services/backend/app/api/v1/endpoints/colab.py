    ...
    with tempfile.TemporaryDirectory(prefix="skolab-tex-") as raw_dir:
        ...
        try:
            ...
            return CompileResponse(
                status="timeout",
                log=log,
                errors=["Compilation exceeded the 20 second limit."],
            )
        ...
        return CompileResponse(
            status="compiled", pdf_base64=base64.b64encode(pdf).decode("ascii"), log=log
        )


@router.post("/colab/compile", response_model=CompileResponse)
async def compile_latex(
    req: CompileRequest, _user: dict = Depends(require_firebase_user)
) -> CompileResponse:
