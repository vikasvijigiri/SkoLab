...
class AuthorService:
    ...
    async def get_author_suggestions(self, query: str) -> List[AuthorSuggestion]:
        ...
        # Try DB first
        pg_suggestions = await AuthorRepository.pg_search_suggestions_metrics(
            self.session, query, limit=4
        )
        if not pg_suggestions:
            pg_suggestions = await AuthorRepository.pg_search_suggestions_profiles(
                self.session, query, limit=4
            )
        ...
        if len(pg_suggestions) >= 4:
            suggestions = [
                AuthorSuggestion(
                    ...
                    if hasattr(r, "current_institution")
                    else getattr(r, "institution", "Independent"),
    ...
    async def search_author(
        self, name: str, author_id: str = None
    ) -> Union[AuthorResponse, dict]:
        clean_id = author_id.split("/")[-1] if author_id else None

        # Check PG
        if clean_id:
            pg_row = await AuthorRepository.pg_get_researcher_metrics(
                self.session, clean_id
            )
            if pg_row:
                works = await AuthorRepository.pg_get_researcher_works(
                    self.session, clean_id
                )
