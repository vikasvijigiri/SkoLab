...
export function GoogleSignInButton({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "flex h-12 w-full cursor-pointer items-center justify-center gap-3 rounded-md border border-border bg-white font-body text-body font-medium text-[#1f1f1f] transition-[opacity,box-shadow,border-color] duration-[var(--motion-fast)] hover:border-text-muted/40 hover:shadow-card disabled:cursor-not-allowed disabled:opacity-70"
      )}
      style={{ transitionTimingFunction: "var(--ease-standard)" }}
    >
      {loading ? (
        <Spinner size={18} className="text-[#1f1f1f]/70" />
      ) : (
