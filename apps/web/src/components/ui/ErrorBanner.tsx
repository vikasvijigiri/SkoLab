...
export function ErrorBanner({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div className="flex items-center gap-3 rounded-md border border-notification/30 bg-notification/10 px-3.5 py-3">
      <TriangleAlert size={16} className="shrink-0 text-notification" />
      <p className="min-w-0 flex-1 font-body text-body-s leading-relaxed text-text-secondary">{message}</p>
      {onRetry && (
        <button
          onClick={onRetry}
