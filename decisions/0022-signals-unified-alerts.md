...
Implemented on a worktree branched from `main` (not yet merged into
`feature/frontend-redesign-2026-09` or `main`) — see `HANDOFF.md` for the
branch. `ActivityType` gained all five new kinds; `useNotifications.ts` and
`NotificationsBell.tsx` render each with real, specific copy (never a
...
- **`tracked_researcher_paper`** — `trackedResearcherPapers` reads
  ...
  author's recent OpenAlex works. The consumer is real and tested, but
  Discovery's Track feature (decisions/0021) that writes that collection is
  itself still unimplemented, so in practice there is nothing to track yet —
  this closes half the loop and is ready the moment the other half ships.
