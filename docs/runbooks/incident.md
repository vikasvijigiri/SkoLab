...
### E. OpenAlex Upstream API Failure Response
> **Added:** Post-mortem CA-003 from inc-2026-001. Apply this procedure immediately when `OpenAlexApiHighErrorRate` alert fires or users report 502 errors on search routes.

1. **Verify OpenAlex Upstream Status:**
   Check the OpenAlex public status page: `https://openalex.org/`

2. **Check Error Rate in Prometheus:**
   ```promql
   increase(outbound_http_requests_total{status=~"5.."}[2m])
   ```
   If >10 in 2 minutes — OpenAlex is likely down.

3. **Verify Cache Coverage:**
   ```sql
   SELECT COUNT(*) FROM suggestions_cache WHERE created_at > NOW() - INTERVAL '1 hour';
   SELECT COUNT(*) FROM profile_cache WHERE created_at > NOW() - INTERVAL '1 hour';
   ```

4. **Apply Kill Switch for Search Routes:**
   ```bash
   KILL_SWITCHES=papers/search,authors/search,collaborators/suggest
   ```
   Restart/reload the backend service. Affected routes return HTTP 503 with informative message.

5. **Post Status Update:** Use Section 6 outage template. Mention cached profiles remain available.
...
### F. LLM Rate Limit Exhaustion Response
> **Added:** Post-mortem CA-005 from inc-2026-002. Apply when `LlmApiQuotaApproaching` alert fires or agent chat routes return HTTP 429.

1. **Identify Exhausted Provider:**
   ```bash
   docker logs skolab_backend 2>&1 | grep -i "rate.limit\|429\|quota"
   ```

2. **Verify OpenRouter Fallback Is Active:**
   ```bash
   docker logs skolab_backend 2>&1 | grep -i "openrouter\|fallback"
   ```
   If OpenRouter is serving responses — **no immediate action needed**. Monitor and wait for quota reset.

3. **Check Prometheus Quota Metric (if metric is configured):**
   ```promql
   groq_tokens_remaining_ratio
   ```
