...
| **2** ✅ | `/daily_feed/dismiss` (owner check ported to Go), `/support/metrics`, `/zotero/*` stubs → Go `internal/feed` — **PR #32, CI green** (`go build` + `go test -race` + pytest 166). `daily_feed` / `daily_conjecture` / roadmap / `industry_opportunities` / `industry_academic_tieups` **stay** Python (LLM/embeds), reached via `NoRoute`. `docs/plans/2026-09-04-phase2-feed-to-go.md`. | Go | split generate-vs-store cleanly |
| **3** ◑ | `RateLimiter` / `TokenBucket` **deleted** — gateway owns per-IP limiting; `MetricsStore` + `GET /metrics` **deleted** from Python + `security_guard_middleware` slimmed to the kill-switch (device-sig + admin gate removed) — `docs/plans/2026-09-04-retire-python-infra.md`. **Still open:** gateway `prometheus/client_golang` endpoint (`[NEEDS DECISION]`), `PgBackedCache` L2 → Upstash Redis. Substring WAF already gone (#29). | Upstash, Cloudflare, Prometheus/OTel | cache-key parity; metric continuity |
