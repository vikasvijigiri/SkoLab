...
| **2** ✅ | `POST /daily_feed/dismiss` (owner check + dismissed-ids write), `GET /support/metrics`, `/integrations/zotero/*` stubs → Go `internal/feed`. Feed **generation** (`GET /daily_feed` + the `daily_conjecture` / `roadmap` / `industry_opportunities` / `industry_academic_tieups` LLM routes) stays Python, reached via `NoRoute`. `docs/plans/2026-09-04-phase2-feed-to-go.md`. Go = **UNVERIFIED** (no toolchain / CI). | Go | split generate-vs-store cleanly |
