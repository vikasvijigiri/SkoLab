...
if ($devices -eq $null -or $devices.Count -eq 0) {
    Write-Host "❌ No connected ADB devices found. Please plug in your phone via USB or connect via wireless debugging (adb connect <ip>:<port>)." -ForegroundColor Red
    exit 1
}

# Use the first active device
$targetDevice = $devices[0]
Write-Host "📱 Target Device Detected: $targetDevice" -ForegroundColor Green
