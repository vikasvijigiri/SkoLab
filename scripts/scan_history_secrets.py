...
def main():
    ...
    for commit in commits:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if line.startswith("+") and not line.startswith("+++"):
                ...
                if "env.example" in current_file.lower() or "test_" in current_file.lower() or "/tests/" in current_file.lower():
                    continue

                for name, pattern in PATTERNS.items():
                    if pattern.search(content):
                        # Avoid matching pattern definitions inside our scanners
                        if "detect_secrets.py" in current_file or "scan_history_secrets.py" in current_file:
                            continue
    ...
    if found_secrets:
        ...
        for secret in found_secrets:
            print(f"  - Commit: {secret['commit'][:8]} | File: {secret['file']} | Issue: Detected {secret['type']} | Author: {secret['author']} | Date: {secret['date']}")
        sys.exit(1)

    print("Historical secrets scan completed. Zero leaked credentials detected in Git history.")
    sys.exit(0)
