...
def main():
    parser = argparse.ArgumentParser(description="Audit SSL/TLS configurations and certificate expiration dates.")
    parser.add_argument("--domain", default="google.com", help="Target domain to audit (default: google.com)")
    ...
    print(f"  - Cipher Suite       : {cipher[0]} (Protocol: {cipher[1]} | Strength: {cipher[2]} bits)")
    
    print("\n=== Certificate Expiration Audit ===")
    if days_to_expire < 0:
        print(f"[CRITICAL] Certificate has EXPIRED! (Expired {-days_to_expire} days ago)")
        sys.exit(1)
    elif days_to_expire < 30:
        print(f"[WARNING] Certificate is expiring soon! (Expires in {days_to_expire} days)")
        sys.exit(1)
    else:
