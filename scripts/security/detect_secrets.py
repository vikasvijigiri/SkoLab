...
def main():
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        if line.startswith("+") and not line.startswith("+++"):
            ...
            if (
                ...
                or "scripts/" in current_file.lower()
                or "detect_secrets" in current_file.lower()
            ):
                continue
            ...
            for name, pattern in PATTERNS.items():
                if pattern.search(content):
                    found_secrets.append(
                        {"file": current_file, "line": line_num, "type": name}
                    )
    ...
    if found_secrets:
        ...
        for secret in found_secrets:
            print(
                f"  - File: {secret['file']} | Diff Line: {secret['line']} | Issue: Detected {secret['type']}"
            )
        print("\nCommit aborted.")
        sys.exit(1)
