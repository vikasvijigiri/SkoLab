...
def main():
    ...
    color_kt_path = os.path.join(root_dir, "android-app", "app", "src", "main", "java", "com", "company", "skolab", "ui", "theme", "Color.kt")
    spacing_kt_path = os.path.join(root_dir, "android-app", "app", "src", "main", "java", "com", "company", "skolab", "ui", "theme", "Spacing.kt")
    shape_kt_path = os.path.join(root_dir, "android-app", "app", "src", "main", "java", "com", "company", "skolab", "ui", "theme", "Shape.kt")
    motion_kt_path = os.path.join(root_dir, "android-app", "app", "src", "main", "java", "com", "company", "skolab", "ui", "theme", "Motion.kt")
    ...
    if os.path.exists(spacing_kt_path):
        ...
        for key, val in tokens["spacing"].items():
            ...
            if not re.search(pattern, spacing_content):
                errors.append(f"Spacing token '{key}' with value '{val}' not found in Spacing.kt")
    else:
    ...
    if os.path.exists(shape_kt_path):
        ...
        for key, val in tokens["shapes"].items():
            ...
            if not re.search(pattern, shape_content):
                errors.append(f"Shape token '{key}' with value '{val}' not found in Shape.kt")
    else:
    ...
    if os.path.exists(motion_kt_path):
        ...
        for key, val in tokens["motion"].items():
            ...
            if not re.search(pattern, motion_content):
                errors.append(f"Motion token '{key}' with value '{val}' not found in Motion.kt")
    else:
    ...
    if os.path.exists(color_kt_path):
        ...
        for name, val in tokens["colors"].items():
            ...
            if pattern_light not in content_upper or pattern_dark not in content_upper:
                errors.append(f"Color token '{name_upper}' (Light: {light_hex}, Dark: {dark_hex}) does not match or is missing in Color.kt")
    else:
