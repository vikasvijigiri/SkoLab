...
async def run_checks():
    ...
    async with engine.connect() as conn:
        ...
        try:
            ...
            res = await conn.execute(
                text("SELECT cache_key, expires_at FROM cache_entries;")
            )
            cache_rows = res.fetchall()
            print(
                f"[PASS] Cache entries table checked. Total cache records: {len(cache_rows)}"
            )
            ...
            for row in cache_rows:
                ...
                if key.startswith("history_summary::") or key.startswith(
                    "user_memory::"
                ):
                    user_id = key.split("::")[-1]
            ...
            if expired_cache_count > 0:
                print(
                    f"[WARN] Found {expired_cache_count} expired cache entries in DB."
                )
                warnings += 1
            ...
            if expired_cache_count == 0 and orphaned_cache_count == 0:
                print(
                    "[PASS] Local cache consistency verified: cache records validate with database states."
                )
        except Exception as e:
