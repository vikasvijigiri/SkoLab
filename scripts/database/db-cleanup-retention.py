...
DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    "postgresql+asyncpg://postgres:postgrespassword@localhost:5432/skolab",
)
...
async def offload_and_prune():
    ...
    cutoff_date = datetime.datetime.now(datetime.timezone.utc).replace(  # noqa: UP017 - datetime.UTC is 3.11+, this script runs under CI Python 3.10
        tzinfo=None
    ...
    async with engine.connect() as conn:
        for table in tables_to_prune:
            try:
                ...
                timestamp = datetime.datetime.now(datetime.timezone.utc).strftime(  # noqa: UP017 - datetime.UTC is 3.11+, this script runs under CI Python 3.10
                    "%Y%m%d_%H%M%S"
                )
                ...
                print(
                    f"[PASS] Offloaded {len(rows)} records from '{table}' to {file_path}"
                )
                ...
                await conn.commit()
                print(
                    f"[PASS] Pruned {del_res.rowcount} rows from '{table}' in database."
                )
        ...
        now_utc = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)  # noqa: UP017 - datetime.UTC is 3.11+, this script runs under CI Python 3.10
        for table, col in tables_with_expiry:
            try:
                ...
                if del_res.rowcount > 0:
                    print(
                        f"[PASS] Pruned {del_res.rowcount} expired rows from '{table}'."
                    )
            except Exception as e:
        ...
        try:
            ...
            if res.rowcount > 0:
                print(
                    f"[PASS] Resolved {res.rowcount} orphaned user settings relations."
                )
            ...
            if res.rowcount > 0:
                print(
                    f"[PASS] Resolved {res.rowcount} orphaned chat history relations."
                )
        except Exception as e:
