...
class LLMResponse:
    def __init__(self, content: Optional[str], tool_calls: Optional[List[Dict[str, Any]]] = None, model_used: str = "") -> None:
...
class LLMService:
    ...
    async def query(
        ...
        for model in models_to_try:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    print(f"[LLMService] Groq model {model} returned status {resp.status_code}: {resp.text[:200]}", flush=True)
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("[LLMService] All Groq models failed. Attempting OpenRouter fallback...", flush=True)
            try:
                try:
                    return await self.query_openrouter(
                        messages=messages,
        ...
        raise Exception("LLM query failed across all Groq models, and no OpenRouter fallback available.")
