...
class AgentService:
    def __init__(self, history_summary_cache, db: Optional[AsyncSession] = None):
    ...
    async def _pg_get_summary(self, cache_key: str):
    ...
    async def _pg_set_summary(self, cache_key: str, summary: str, ttl_hours: int = 12):
    ...
    async def process_agent_chat(self, req, base_url: str = None) -> dict:
        try:
            ...
            if req.user_memory:
                ...
                last_topic = (
                    m.get("lastActiveTopic") or m.get("last_active_topic") or ""
                )
                reading_pace = m.get("readingPace") or m.get("reading_pace") or ""
                collaborators = (
                    m.get("frequentCollaborators")
                    or m.get("frequent_collaborators")
                    or []
                )
                unfinished = (
                    m.get("unfinishedPapers") or m.get("unfinished_papers") or []
                )
                searches = (
                    m.get("frequentSearchTerms") or m.get("frequent_search_terms") or []
                )
                ...
                if reading_pace:
                    parts.append(
                        f"Reading style: {reading_pace} (~{int(avg_read)} min/paper avg)"
                    )
                if collaborators:
            ...
            total_history_tokens = sum(
                self.estimate_tokens(h.get("content") or "") for h in req.history
            )
            ...
            if total_history_tokens < 10000:
                ...
                summary_text = await self._pg_get_summary(older_hash)
                if not summary_text:
                    summary_text = await self.history_summary_cache.get(older_hash)
                if not summary_text:
                    print(
                        f"[AgentChat] Cache miss for history summary. Generating summary for {len(older_messages)} older messages...",
                        flush=True,
                    )
                    summary_prompt = AGENT_SUMMARY_PROMPT
                    for h in older_messages:
                        role_label = (
                            "Researcher" if h.get("role") == "user" else "Ask Skolar"
                        )
                    ...
                    try:
                        ...
                        if res_obj.content:
                            ...
                            await self._pg_set_summary(older_hash, summary_text)
                            await self.history_summary_cache.set(
                                older_hash, summary_text
                            )
                            print(
                                "[AgentChat] Successfully generated and cached conversation summary.",
                                flush=True,
                            )
                        else:
                            print(
                                "[AgentChat] Summarization LLM returned empty content.",
                                flush=True,
                            )
                            raise ValueError(
                                "Summarization LLM returned empty content."
                            )
                    except Exception as e:
                        print(
                            f"[AgentChat] Failed to generate conversation summary: {e}.",
                            flush=True,
                        )
                        raise ValueError(
            ...
            # Escape XML tags to prevent breakout from structural packaging
            sanitized_message = req.message.replace("<", "&lt;").replace(">", "&gt;")
            ...
            if is_llm_working():
                ...
                for turn in range(max_turns):
                    ...
                    try:
                        ...
                        if turn == max_turns - 1:
                            temp_messages.append(
                                {"role": "system", "content": AGENT_FINAL_TURN_PROMPT}
                            )

                        print(
                            f"[AgentChat] Requesting LLM query via LLMService on turn {turn + 1}...",
                            flush=True,
                        )
                        ...
                        if response.content or response.tool_calls:
                            ...
                            success = True
                            print(
                                f"[AgentChat] Turn {turn + 1} succeeded with model: {chosen_model}",
                                flush=True,
                            )
                    except Exception as e:
                    ...
                    if not success or not response_msg:
                        print(
                            f"[AgentChat] Turn {turn + 1} failed completely across all models and OpenRouter.",
                            flush=True,
                        )
                        return {
                    ...
                    if (
                        not tool_calls
                        and "content" in response_msg
                        and response_msg["content"]
                        and "<function=" in response_msg["content"]
                    ):
                    ...
                    if tool_calls and turn < max_turns - 1:
                        ...
                        for tc in tool_calls:
                            ...
                            try:
                                ...
                                print(
                                    f"[AgentChat] Error parsing tool arguments: {e}",
                                    flush=True,
                                )
                                args = {}

                            tool_result = await execute_tool_call(
                                func_name, args, base_url=base_url
                            )
                            messages.append(
                                {
                                    "role": "tool",
                        ...
                        reply = re.sub(
                            r"<function=.*?>.*?</function>", "", reply, flags=re.DOTALL
                        )
