    ...
    async def process_agent_chat(self, req) -> dict:
        try:
            base_prompt = (
                ...
                "You have access to tools that can fetch data from arXiv, Google Scholar, ResearchGate, OpenAlex, and search/scrape the live web. "
                "CRITICAL SEARCH HIERARCHY RULES:\n"
                "1. FIRST: You must try to fetch information from the OpenAlex API using 'search_openalex_author_and_works' or other OpenAlex tools. "
                "If results are returned, present the information immediately.\n"
                "2. SECOND (FALLBACK): If there are NO results from OpenAlex (e.g., the researcher is not indexed, the query fails, or the API is rate-limited/exhausted), "
