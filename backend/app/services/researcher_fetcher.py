...
class PhysicsResearcherFetcher:
    ...
    async def search_arxiv_physicists(self, query: str = "cat:physics*", max_results: int = 50) -> List[str]:
        """
    ...
    async def get_researcher_details(self, author_name: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            recent_papers = sorted(all_papers, key=lambda x: x.get("year") or 0, reverse=True)[:5]
            
            return {
                "name": author_data.get("name"),
                "semantic_scholar_id": author_id,
                "orcid": author_data.get("externalIds", {}).get("ORCID"),
                "institution": author_data.get("affiliations")[0] if author_data.get("affiliations") else "Unknown",
                "last_5_articles": [p.get("title") for p in recent_papers],
                "academic_history": author_data.get("affiliations", []), # Simplified history
