...
def _build_yearly_citations(counts_by_year: List[Dict[str, Any]]) -> List[int]:
    return [c.get("cited_by_count", 0) for c in sorted(counts_by_year, key=lambda x: x.get("year", 0))]
    ...
    async with AsyncSessionLocal() as session:
        try:
            result = await session.execute(
                select(ResearcherMetrics).where(ResearcherMetrics.openalex_id == clean_id)
            )
            ...
            logger.error("[teleport] PG researcher_metrics write failed for %s: %s", clean_id, exc)
            await session.rollback()


async def _pg_upsert_researcher_works(clean_id: str, works: List[Dict[str, Any]]) -> None:
    """
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            await session.execute(
                sa_delete(ResearcherWork).where(ResearcherWork.author_openalex_id == clean_id)
            )
            ...
            logger.error("[teleport] PG researcher_works write failed for %s: %s", clean_id, exc)
            await session.rollback()
...
def _firestore_save_researcher(clean_id: str, doc_payload: Dict[str, Any]) -> None:
    ...
    if db is None:
        logger.info("[teleport] Firestore unavailable — skipping cloud persist for %s", clean_id)
        return
    try:
        from firebase_admin import firestore as _fs
        db.collection("global_researchers").document(clean_id).set(doc_payload, merge=True)
...
async def teleport_researcher(author_id: str) -> None:
    ...
    try:
        ...
        for aff in affiliations:
            ...
            if inst_name and years:
                ...
                if existing is None:
                    ...
                    hist_map[inst_name] = [
                        min(existing[0], min(years)), max(existing[1], max(years))
                    ]
        academic_history = [
        ...
        future_impact       = ms.calculate_future_impact(
            ...
            journal_score=journal_score, h_index=h_index,
        )
        ...
        innovation_score = round(
            abs(disruption_score) * 10 + min(abs(citation_accel), 10)
            + future_impact * 0.5 + network_centrality * 0.3
            + semantic_novelty * 0.3 + interdisciplinary * 0.2
            + collab_diversity * 0.2 + research_consist * 0.1 + open_science * 0.1,
            1,
        )

        logger.info(
            "[teleport] Metrics computed for %s — disruption=%.3f, novelty=%.1f, impact=%.1f",
            display_name, disruption_score, semantic_novelty, future_impact,
        )
                ...
                next_prediction = await ps.predict_next_problem(
                    ...
                    works=[
                        {"title": w.get("title",""), "year": w.get("year"),
                         "citations": w.get("citations",0), "abstract": w.get("abstract","")}
                        for w in works[:10]
                    ],
                )
        ...
        await _pg_upsert_researcher_metrics(clean_id, {
        ...
        works_payload = [
            {
                ...
                "journal": w.get("journal"), "is_open_access": w.get("is_open_access", False),
                "citations": w.get("citations", 0),
                "creativity_score": avg_creativity, "complexity_score": avg_complexity,
                "impact_factor": w.get("impact_factor", 0.0),
                "disruption_score": disruption_score, "semantic_novelty": semantic_novelty,
                "open_science_score": open_science,
            }
            for w in works[:50]
        ]

        _firestore_save_researcher(clean_id, {
        ...
        logger.info("[teleport] ✓ Enrichment complete for %s in %.2fs", display_name, elapsed)
