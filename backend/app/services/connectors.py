    ...
    try:
        async with httpx.AsyncClient(
            ...
            if res.status_code == 200:
                ...
                if results:
                    return "[arXiv PUBLICATIONS FOUND]\n\n" + "\n\n---\n\n".join(
                        results
                    )
...
async def search_google_scholar_profile(query: str) -> str:
    ...
    try:
        ...
        if not profile_url:
            ...
            return (
                "No Google Scholar citations profile found. Here are relevant publications matches:\n\n"
                + "\n\n".join(res_str)
            )
        ...
        async with httpx.AsyncClient(
            ...
            if res.status_code == 200:
                ...
                affiliation = (
                    re.sub("<[^>]+>", "", aff_match.group(1)).strip()
                    if aff_match
                    else "Unknown"
                )
                ...
                for idx, row in enumerate(rows[:15]):
                    ...
                    title = (
                        re.sub("<[^>]+>", "", t_match.group(1)).strip()
                        if t_match
                        else "Untitled"
                    )

                    authors_match = re.search(
                        r'<div class="gs_gray"[^>]*>(.*?)</div>', row
                    )
                    authors = (
                        re.sub("<[^>]+>", "", authors_match.group(1)).strip()
                        if authors_match
                        else "Unknown"
                    )

                    cite_match = re.search(
                        r'class="gsc_a_ac gs_ibl"[^>]*>(.*?)</a>', row
                    )
                    ...
                    pub_list.append(
                        f"  - Title: {title} ({year}) | Citations: {citations} | Authors: {authors}"
                    )

                pubs_str = (
                    "\n".join(pub_list)
                    if pub_list
                    else "  - No publication works found on profile."
                )
...
async def search_researchgate_profile(query: str) -> str:
    ...
    try:
        ...
        return (
            "No direct ResearchGate links found in top matches. Here are relevant search results:\n\n"
            + "\n\n".join(res_str)
        )
    except Exception as e:
...
def generate_downloadable_table(
    headers: List[str], rows: List[List[Any]], filename: str, base_url: str = None
) -> str:
...
def export_bibtex_file(
    publications: List[Dict[str, Any]], filename: str, base_url: str = None
) -> str:
    ...
    try:
        ...
        for idx, pub in enumerate(publications):
            ...
            # Generate a key
            first_author = (
                authors.split(" and ")[0].split()[-1] if authors else "author"
            )
            clean_first_author = "".join(c for c in first_author if c.isalnum()).lower()
            clean_title_first_word = (
                "".join(c for c in title.split()[0] if c.isalnum()).lower()
                if title.split()
                else "work"
            )
...
def generate_research_report(
    title: str, sections: List[Dict[str, Any]], filename: str, base_url: str = None
) -> str:
    ...
    try:
        ...
        md_content.append("*Generated by Ask Skolar*\n")
        md_content.append("---")
...
async def fetch_and_summarize_paper(query: str, source: str = "auto") -> str:
    ...
    is_arxiv_id = bool(
        re.match(r"^(\d{4}\.\d{4,5}|[a-z\-]+/\d{7})(v\d+)?$", query.strip().lower())
    )
            ...
            if is_doi:
                ...
                async with httpx.AsyncClient(
                    ...
                    if r.status_code == 200:
                        ...
                        oa_url = (w.get("primary_location") or {}).get(
                            "landing_page_url", ""
                        )
                        authors = ", ".join(
                            [
                        ...
                        concepts = ", ".join(
                            [c["display_name"] for c in (w.get("concepts") or [])[:8]]
                        )
                ...
                if r.status_code != 200:
                    ...
                    if not raw.strip():
                        ...
                        for entry in entries:
                            ...
                            authors_str = ", ".join(
                                [a.text or "" for a in authors_els[:6]]
                            )
                            cats = [
                                c.attrib.get("term", "")
    ...
    if is_url and source in ("auto", "url"):
        try:
            async with httpx.AsyncClient(
                ...
                r = await client.get(
                    query.strip(), headers={"User-Agent": "Mozilla/5.0"}
                )
                if r.status_code == 200:
                    html = r.text
                    # Strip HTML tags
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "The search query (author name or keyword), e.g. 'Albert Einstein' or 'quantum entanglement'",
                    },
                    "max_results": {
                        "type": "integer",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "The author's name, e.g. 'Albert Einstein'",
                    }
                },
                "required": ["query"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "The author's name, e.g. 'Kush Saha'",
                    }
                },
                "required": ["query"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "The search query, e.g., 'quantum entanglement'",
                    },
                    "max_results": {
                        "type": "integer",
                        "description": "Max results to return (default 5)",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "The search query, e.g., 'React 19 release date'",
                    },
                    "max_results": {
                        "type": "integer",
                        "description": "Max results to return (default 5)",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "The author's name, e.g. 'Albert Einstein' or 'Kush Saha'",
                    },
                    "domain": {
                        "type": "string",
                        "description": "Optional research domain to filter by, e.g. 'Physics', 'Machine Learning'. Use the user's research focus by default.",
                    },
                },
                "required": ["query"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "author_id": {
                        "type": "string",
                        "description": "The OpenAlex author ID, e.g. 'https://openalex.org/A5020214245' or just 'A5020214245'",
                    },
                    "limit": {
                        "type": "integer",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "domain": {
                        "type": "string",
                        "description": "Optional research domain to filter by, e.g. 'Physics'. Use the user's research focus by default.",
                    },
                },
                "required": ["query"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "domain": {
                        "type": "string",
                        "description": "Research domain to disambiguate the author, e.g. 'Physics', 'Computer Science'. ALWAYS pass the user's domain here unless explicitly told otherwise.",
                    },
                },
                "required": ["query"],
            },
...
async def execute_tool_call(
    tool_name: str, arguments: dict, base_url: str = None
) -> str:
    ...
    try:
        if tool_name == "generate_downloadable_table":
            ...
            return await fetch_and_summarize_paper(
                arguments.get("query", ""), arguments.get("source", "auto")
            )
            ...
            authors = await openalex.search_authors(
                arguments.get("query", ""), per_page=20
            )
            if not authors:
            ...
            if domain:
                filtered = [
                    ...
                    for a in authors
                    if any(
                        domain in (c.get("display_name") or "").lower()
                        for c in (a.get("x_concepts") or [])
                    )
                ]
                authors = (
                    filtered if filtered else authors
            ...
            for a in authors[:20]:
                ...
                top_concepts = ", ".join(
                    [c.get("display_name", "") for c in (a.get("x_concepts") or [])[:4]]
                )
                res_list.append(
            ...
            works = await openalex.fetch_author_works(
                arguments.get("author_id", ""), per_page=arguments.get("limit", 10)
            )
            if not works:
            ...
            works = await openalex.search_works(
                query, per_page=arguments.get("limit", 10)
            )
            if not works:
            ...
            for w in works:
                ...
                citations = w.get("cited_by_count", 0)
                concepts = ", ".join(
                    [c.get("display_name", "") for c in (w.get("concepts") or [])[:4]]
                )
                res_list.append(
                    f"Title: {title}\nYear: {year}\nDOI: {doi}\nCitations: {citations}\nFields: {concepts}"
                )
            ...
            domain = arguments.get("domain", "").lower().strip()
            authors = await openalex.search_authors(
                arguments.get("query", ""), per_page=15
            )
            if not authors:
                return (
                    f"No authors found on OpenAlex for query: {arguments.get('query')}"
                )
            ...
            for idx, a in enumerate(authors_sorted[:3]):  # take top 3 domain-matched
                ...
                h_index = stats.get("h_index", 0) if stats else 0
                top_concepts = ", ".join(
                    [c.get("display_name", "") for c in (a.get("x_concepts") or [])[:5]]
                )
                profile_info = (
                ...
                for w in works:
                    ...
                    citations = w.get("cited_by_count", 0)
                    works_list.append(
                        f"  - Title: {title} ({year}) | Citations: {citations} | DOI: {doi}"
                    )

                works_str = (
                    "\n".join(works_list)
                    if works_list
                    else "  - No publication works found."
                )
